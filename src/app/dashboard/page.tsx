...
type OrderPlat = {
  ...
  total_amount: number
  traiteurs?: { name: string } | null
  order_items?: { id: string; quantity: number; dishes?: { name: string } | null }[]
}
...
export default async function DashboardPage() {
  ...
  const { data: commandesTraiteur } = await supabase
  ...
  const { data: ordersPlats } = await supabase
    .from('orders')
  ...
  const { data: gpRequests } = await supabase
    .from('gp_requests')
    .select('*, gp_listings(departure_city, arrival_city, departure_country, arrival_country)')
  ...
  const getStatutBadge = (statut: string) => {
    switch (statut) {
      ...
      case 'acceptee':
      case 'accepted':
        return <span className="text-xs bg-[#E8F5E9] text-[#1D6B45] px-2 py-0.5 rounded-full">✅ Acceptée</span>
      case 'refusee':
      case 'rejected':
        return <span className="text-xs bg-red-50 text-red-500 px-2 py-0.5 rounded-full">❌ Refusée</span>
      default:
        return null
    }
  }
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
            ...
            <div className="bg-[#D4870A]/10 border border-[#D4870A]/30 rounded-2xl p-4 mb-6 flex items-center justify-between hover:bg-[#D4870A]/15 transition-all">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Cliquez pour accepter ou refuser</p>
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 mb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/commandes" className="text-xs text-[#1D6B45] font-medium hover:underline">
              Voir tout →
            </Link>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {formatDate(cmd.created_at)} · {cmd.nb_personnes} pers.
                      </p>
                    </div>
                  </div>
              ...
              {gpRequests?.map((req: GpRequest) => (
                <div key={req.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-800">
                        {req.gp_listings?.departure_city} → {req.gp_listings?.arrival_city}
                      </p>
                      <p className="text-xs text-gray-400">
