  ...
  if (success) return (
    <div className="min-h-screen bg-[#FAF7F2] flex items-center justify-center px-4">
      <div className="text-center max-w-sm">
        ...
        <h2 className="text-2xl font-bold text-[#1D6B45] mb-2">Demande envoyée !</h2>
        <p className="text-gray-500 mb-6">
          {listing.profiles?.full_name || 'Le GP'} a reçu ta demande et va te contacter rapidement.
        </p>
        {listing.profiles?.whatsapp && (
          <button
            onClick={() => {
              const num = listing.profiles!.whatsapp!.replace(/\+/g, '').replace(/\s/g, '')
              const msg = encodeURIComponent(`Bonjour, je viens d'envoyer une demande de colis sur AfriConnect. ${form.weight_kg}kg — ${form.content_desc}`)
        ...
        <Link href="/commandes" className="text-[#1D6B45] font-medium hover:underline text-sm block mb-2">
          Voir mes commandes →
        </Link>
    ...
    <div className="min-h-screen bg-[#FAF7F2] pb-32">
      ...
      <div className="px-4 py-6 max-w-2xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <h2 className="font-semibold text-gray-800 mb-4">👤 À propos du GP</h2>
          <div className="flex items-center gap-4">
            ...
            {listing.profiles?.whatsapp && (
              <button
                onClick={() => {
                  const num = listing.profiles!.whatsapp!.replace(/\+/g, '').replace(/\s/g, '')
                  const msg = encodeURIComponent(`Bonjour, j'ai vu votre annonce GP sur AfriConnect pour ${listing.departure_city} → ${listing.arrival_city}`)
                  window.open(`https://wa.me/${num}?text=${msg}`, '_blank')
                }}
        ...
        {/* Détails annonce */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <h2 className="font-semibold text-gray-800 mb-4">📦 Détails de l&apos;annonce</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              <p className="text-xs text-gray-400 mb-1">Kg disponibles</p>
              <p className="text-2xl font-bold text-[#1D6B45]">{listing.available_kg}</p>
              <p className="text-xs text-gray-400">kg</p>
            </div>
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              <p className="text-xs text-gray-400 mb-1">Prix</p>
              <p className="text-2xl font-bold text-[#1D6B45]">{listing.price_per_kg}</p>
              <p className="text-xs text-gray-400">€/kg</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={e => setForm(p => ({ ...p, content_desc: e.target.value }))}
            ...
            {form.weight_kg && (
              <div className="bg-[#E8F5E9] rounded-xl p-4 flex justify-between items-center">
                <span className="text-sm font-medium text-[#1D6B45]">Total estimé</span>
                <span className="text-xl font-bold text-[#1D6B45]">{total.toFixed(2)} €</span>
              </div>
            )}
